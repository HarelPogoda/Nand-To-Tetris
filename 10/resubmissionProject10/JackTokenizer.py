    ...
    def __init__(self, input_stream: typing.TextIO) -> None:
        ...
        for line in input_lines:
            ...
            for i in range(len(line)):
                if (line[i] == ";" and i != len(line) - 1) or i == len(line) - 1:
                    end_of_line = i+1
                    separated_lines.append(line[start_of_line : end_of_line])
        ...
        for line in separated_lines:
            ...
            for i in range(len(line)):
                if line[i] == '"' and in_string == False:
                    in_string = True
                elif line[i] == '"' and in_string == True:
                    in_string = False
                if not in_string and i <= len(line) - 2 and line[i] + line[i+1] == "/*":
                    in_comment_block = True
                if not in_string and i <= len(line) - 2 and line[i] + line[i+1] == "//":
                    in_one_line_comment = True
                if not (in_comment_block or in_one_line_comment):
                    clean_line = clean_line + line[i]

                if (i >= 1) and in_comment_block and line[i-1] + line[i] == "*/":
                    in_comment_block = False
